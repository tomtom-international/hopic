...
archival_sig = '$Format:%D$'
try:
    with open(os.path.join(root_dir, '.git_archival.txt')) as f:
        git_archival = f.read()
except FileNotFoundError:
    git_archival = None

if os.path.isfile(os.path.join(root_dir, pkg_name, 'cli.py')) \
        and (os.path.exists(git_dir) or re.search(r'ref-names: .*tag: ', git_archival)):
...
# sphinxcontrib.versioning config
...
# Use greatest tagged version as root/default for documentation
...
htmlhelp_basename = 'HopicCIdriverdoc'
...
man_pages = [
    (master_doc, pkg_name, project + u' Documentation',
     [author], 1)
]
